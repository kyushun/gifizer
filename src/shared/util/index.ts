...
export const toSeconds = (time: number | string) => {
  const numTime = Number(time);
  const match = typeof time == "string" ? time.match(timeFormat) : null;

  if (match) {
    let sec = 0;
    sec += parseInt(match?.groups?.hh || "0") * 3600;
    sec += parseInt(match?.groups?.mm || "0") * 60;
    sec += parseInt(match?.groups?.ss || "0");
    sec += Number("0." + (match?.groups?.ms?.slice(0, 3) || "0"));
    return sec;
  } else if (!isNaN(numTime) && numTime >= 0) {
    return numTime;
  } else {
    return 0;
  }
...
export const toFormattedTime = (time: number | string) => {
  const numTime = Number(time);
  const match = typeof time == "string" ? time.match(timeFormat) : null;

  if (match) {
    return (
      formatTime(match?.groups?.hh || 0) +
      ":" +
      formatTime(match?.groups?.mm || 0) +
      ":" +
      formatTime(match?.groups?.ss || 0) +
      "." +
      formatMs(match?.groups?.ms || 0)
    );
  } else if (!isNaN(numTime) && numTime >= 0) {
    const strTime = numTime.toString();
    const splitedTime = strTime.split(".");
    const secs = parseInt(strTime);
    const ms = splitedTime.length == 2 ? parseInt(splitedTime[1]) : 0;
